...
function makeContext(authorizationHeader: string | undefined): { context: ExecutionContext; request: Record<string, unknown> } {
    const request: Record<string, unknown> = { headers: { authorization: authorizationHeader } };
    const context = {
...
describe('ClerkAuthGuard', () => {
    ...
    it('throws if CLERK_SECRET_KEY is not configured, without attempting verification', async () => {
        ...
        await expect(guard.canActivate(context)).rejects.toThrow('CLERK_SECRET_KEY is not configured');
    ...
    it('attaches the verified Clerk userId to the request and allows the request through', async () => {
        ...
        expect(mockVerifyToken).toHaveBeenCalledWith('good-token', { secretKey: 'test-secret-key' });
    });
});
