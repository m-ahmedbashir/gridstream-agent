...
describe('TelemetryQueueConsumer', () => {
  ...
  beforeEach(() => {
    ...
    } as unknown as AiDiagnosticTriggerService;
  });
  ...
  it('triggers the AI diagnostic when the inserted reading breaches a safety bound', async () => {
    ...
    expect(aiDiagnosticTrigger.trigger).toHaveBeenCalledWith(
      'device-2',
      row,
      'THERMAL_RUNAWAY',
    );
  ...
  it('triggers with VOLTAGE_SAG when a voltage sag (not a thermal breach) fires', async () => {
    const row = {
      id: 'log-4',
      deviceId: 'device-4',
      timestamp: new Date(),
