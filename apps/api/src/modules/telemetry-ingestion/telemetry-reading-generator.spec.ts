import { ANOMALY_PROBABILITY, THERMAL_RUNAWAY_TEMP_C, VOLTAGE_SAG_V, generateReading } from './telemetry-reading-generator';
...
describe('generateReading()', () => {
  it('a BATTERY device reports batterySoC/batteryTempCelsius and no solar production', () => {
    ...
    const reading = generateReading({ id: 'device-1', deviceType: 'BATTERY' }, random);
  ...
  it('a SOLAR device reports solarProductionKwh and no battery fields', () => {
    ...
    const reading = generateReading({ id: 'device-2', deviceType: 'SOLAR' }, random);
  ...
  it('a WALLBOX/HEAT_PUMP device reports neither solar nor battery fields', () => {
    ...
    const reading = generateReading({ id: 'device-3', deviceType: 'WALLBOX' }, random);
  ...
  it('injects thermal runaway (>65°C) for a BATTERY device when the anomaly roll succeeds', () => {
    ...
    const reading = generateReading({ id: 'device-4', deviceType: 'BATTERY' }, random);
  ...
  it('injects a voltage sag (<200V) for a non-BATTERY device when the anomaly roll succeeds', () => {
    ...
    const reading = generateReading({ id: 'device-5', deviceType: 'HEAT_PUMP' }, random);
  ...
  it('never injects an anomaly when the roll lands exactly on the probability boundary', () => {
    ...
    const reading = generateReading({ id: 'device-6', deviceType: 'BATTERY' }, random);
