...
const READING = { id: 'log-1', deviceId: 'device-1', timestamp: new Date() } as any;

describe('AiDiagnosticTriggerService', () => {
    it('delegates to DiagnosticsService.diagnose()', async () => {
        const diagnosticsService = { diagnose: jest.fn().mockResolvedValue({ id: 'fault-1' }) } as unknown as DiagnosticsService;
        const service = new AiDiagnosticTriggerService(diagnosticsService);

        await service.trigger('device-1', READING);

        expect(diagnosticsService.diagnose).toHaveBeenCalledWith('device-1', READING);
    });
    ...
    it('swallows a diagnosis failure rather than propagating it', async () => {
        ...
        await expect(service.trigger('device-1', READING)).resolves.toBeUndefined();
    });
});
